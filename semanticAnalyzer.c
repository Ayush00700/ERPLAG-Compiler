...
void perform_type_matching_in(ast_node* actual, sym_tab_entry* formal, func_entry* curr, int line)
{
    while(actual&&formal)
    {
       ...
       else
       {
            ...
            {
                //The above code might need to change, i we allow array of array
       ...
       actual=actual->next;
       formal=formal->next;
       //This code currently works as compare dtype first checks the datatype. then if false, other things
       //An alternative would be to create a new memory space and do things accordingly 
    }
    if(actual||formal)
    {
        throw_error(PARAMETER_LIST_MISMATCH,line);
