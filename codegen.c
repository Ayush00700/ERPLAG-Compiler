...
void codegen_input(ir_code_node* ir, func_entry* local_ST)
{
    ...
    */
    ...
    // If not array
    if(strcmp(rhsTypeExp->datatype, "array"))
    {
        // Set up format specifier based on data type
        if(!strcmp(resultType, "integer"))
        {
    ...
    else
    {
        ...
        // Static array
        if(rhsTypeExp->is_static)
        {
            int low = rhsTypeExp->arr_data->lower_bound;
            int high = rhsTypeExp->arr_data->upper_bound;
            int num = high - low + 1;
            ...
            fprintf(assembly, "\t\tmov      rax , 1\n");
            fprintf(assembly, "\t\tmov      rdi , 1\n");
            if(!strcmp(rhsTypeExp->arr_data->arr_datatype,"integer"))
            {
                fprintf(assembly, "\t\tmov      rsi , integer\n");
                fprintf(assembly, "\t\tmov      rdx , integer_len\n");
            }

            else if(!strcmp(rhsTypeExp->arr_data->arr_datatype,"real"))
            {
                fprintf(assembly, "\t\tmov      rsi , real\n");
                fprintf(assembly, "\t\tmov      rdx , real_len\n");
            }

            else if(!strcmp(rhsTypeExp->arr_data->arr_datatype,"boolean"))
            {
            ...
            if(!strcmp(rhsTypeExp->arr_data->arr_datatype, "integer"))
            {
                ...
                fprintf(assembly, "\t\trsp_realign\n");
                    // fprintf(assembly, "\t\tmov      rsi , rbp\n");
                    // fprintf(assembly, "\t\tsub      rsi , %d\n",offsetResult);       // offset + 16 so that sub rsi statement can be inside loop    
                    // fprintf(assembly, "\t\txor      rax , rax\n");
            ...
            else if(!strcmp(rhsTypeExp->arr_data->arr_datatype, "real"))
            {
                ...
                fprintf(assembly, "\t\tmov      rcx , %d\n",num);
                fprintf(assembly, "\t\tmov      rsi , rbp\n");
                fprintf(assembly, "\t\tsub      rsi , %d\n",offsetResult+64);       // offset + 16 so that sub rsi statement can be inside loop
                
                fprintf(assembly, "%s:\n",arr_in);
                fprintf(assembly, "\t\tsub      rsi , 64\n");
                fprintf(assembly, "\t\tmov      rax , 1\n");
                fprintf(assembly, "\t\trsp_align        ; align rsp to 16 byte offset boundary\n");
                fprintf(assembly, "\t\tcall     scanf\n");
                fprintf(assembly, "\t\trsp_realign      ; realign it to original position\n");
        ...
        else
        {
            ...
            if(rhsTypeExp->is_static)
            {
                ...
                if(!strcmp(rhsTypeExp->arr_data->arr_datatype, "integer"))
                {
                    ...
                    fprintf(assembly, "\n\t\tpush_regs                                         ; save values\n");
                    fprintf(assembly, "\t\tmov      rdi , fmt_spec_int_arr_out_last                  ; get corresponding format specifier\n");
                ...
                else if(!strcmp(rhsTypeExp->arr_data->arr_datatype, "real"))
                {
                    ...
                    fprintf(assembly, "%s:\n",arr_out);                    
                    fprintf(assembly, "\t\tlea      rdi , [rel fmt_spec_real_arr_out]                  ; get corresponding format specifier\n");
                    fprintf(assembly, "\t\tsub      rsi , 64\n");
                    fprintf(assembly, "\t\tmov      rax , 1\n");
                    fprintf(assembly, "\t\trsp_align                                         ; align stack pointer\n");
                    fprintf(assembly, "\t\tcall     printf                                   ; system call for output\n");
                ...
                else if(!strcmp(rhsTypeExp->arr_data->arr_datatype, "boolean"))
                {
                    ...
                    fprintf(assembly, "%s:\n",arr_out);                    
                    fprintf(assembly, "\t\tlea      rdi , [rel fmt_spec_bool_arr_out]                  ; get corresponding format specifier\n");
                    fprintf(assembly, "\t\tsub      rsi , 16\n");
                    fprintf(assembly, "\t\txor      rax , rax\n");
                    fprintf(assembly, "\t\trsp_align                                         ; align stack pointer\n");
                    fprintf(assembly, "\t\tcall     printf                                   ; system call for output\n");
...
void starter(FILE* assembly_file,ir_code* IR)
{
    ...
    */
    ...
    else
    {
        ...
        fprintf(assembly, "\t\tfmt_spec_int_arr_out: db \"%%d \", 0\n");
        fprintf(assembly, "\t\tfmt_spec_int_arr_out_last: db \"%%d\", 10, 0\n");
        fprintf(assembly, "\t\tfmt_spec_real_in: db \"%%4f\", 0\n");
        fprintf(assembly, "\t\tfmt_spec_real_out: db \"%%4f\", 10, 0\n");
        ...
        fprintf(assembly, "\t\tprint_out: db \"Output: \", 0\n");
        fprintf(assembly, "\t\tprint_out_len: equ $ - print_out\n");
        fprintf(assembly, "\t\tprintError: db \"Error, Array Out of Dynamic Bound  \", 10, 0\n");
        fprintf(assembly, "\t\tprintError_len: equ $ - printError\n");
