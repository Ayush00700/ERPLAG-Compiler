...
void printer(treeNodes* current,FILE* fp){
    num_nodes++;
    if(current->symbol->isTerminal){
        if(!strcmp(current->symbol->nodeInfo,"eps")){
            fprintf(fp,"<Lexeme:---->\n");
        }else{
        fprintf(fp,"TK[<Type:%s> <Lexeme:%s> <Line Number:%d>] ",current->token->type,current->token->lexeme,current->token->line_no);
        if(!strcmp(current->token->type,"RNUM")){
            fprintf(fp,"<Values :%f>\n",current->token->values.rnum);
        }
        else if(!strcmp(current->token->type,"NUM")) {
...
void InOrderTraversal(treeNodes* current,FILE* fp){
    int flag = 0;
    if(!current->child){
        printer(current,fp);
        if(current->r_sibling!=NULL){
            InOrderTraversal(current->r_sibling,fp);
        }
        return;
    }
    else{
        InOrderTraversal(current->child,fp);
        printer(current,fp);
        if(current->r_sibling!=NULL){
            InOrderTraversal(current->r_sibling,fp);
        }
...
/*This function gives cue to start the Syntax Analyzer*/
{
    ...
    FILE* fp = fopen("parsing_errors.txt","w");
    ...
    while(curr)    
    {
                ...
                if(!val){
                    ...
                    // else
                   {
                    ...
                    if(strcmp(stackTop->nodeInfo, parse_stack.top->nodeInfo)==0)
                    {
                        ...
                        fprintf(fp,KMAG"Line No. %d\t\t"RESET"EVENT -- Expected %s but got %s\t\t\t\t"KGRN"ACTION -- Ignored %s\n"RESET,curr->line_no,stackTop->nodeInfo,curr->type,curr->type);
                ...
                // If lookahead symbol is in FOLLOW(A)
                    ...
                    if(stackTop->nextNode->isTerminal){
                        ...
                        if(!strcmp(stackTop->nextNode->nodeInfo,curr->type )){
                            ...
                            fprintf(fp,KMAG"Line No. %d\t\t"RESET"EVENT -- Expected %s statement insted got %s\t\t\t\t"KGRN"ACTION -- Popped stack top\n"RESET,curr->line_no,stackTop->nodeInfo,curr->type);
                            PARSER_ERRORS = 1;
        ...
        // the case when it goes out of the 4above statement i.e. there is a terminal to match
        {
            ...
            else
            {
                ...
                PARSER_ERRORS = 1;
                fprintf(fp,KMAG"Line No. %d\t\t"RESET"EVENT -- Terminal mismatch %s\t\t\t\t"KGRN"ACTION -- Popped stack top\n"RESET,curr->line_no,stackTop->nodeInfo);
                ...
                stackTop=top(&parse_stack);
                if(stackTop==NULL){
                    fprintf(fp,"Encountered end of token $.. exiting");
                    return ;
                }
                goToNextRight();
